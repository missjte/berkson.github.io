-- Copyright 2015 Prick Your Finger
--
...
readPost fname = makePost <$> readFile fname
  where makePost = P.parse (takeBaseName fname)
...
writePosts tmpl ctx posts config = snd <$> foldM writePost (1, []) withRelated
...
writeIndex globalContext = writePage 0 "/" context
  where context = M.unions [ Template.stringField "title"     "Prick Your Finger"
...
main = do
  ...
  let yctx          = Template.stringField "year" $ show year
      ...
      config        = Config { outDir   = "out/"
                             , imageDir = "journal/images/compressed/" }

  createDirectoryIfMissing True  "out/images/"
  copyFiles "journal/images/compressed/" "out/images/"
